...
```bash
image-to-text$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
image-to-text$ sam local start-api
image-to-text$ curl http://localhost:3000/classify_digit
```
...
```bash
image-to-text$ sam logs -n InferenceFunction --stack-name "image-to-text" --tail
```
...
```bash
sam delete --stack-name "image-to-text"
```
